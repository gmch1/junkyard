...
- 菜单栏只保留“打开管理页面”和“退出”两个入口。
- 从网页复制局域网 Base URL、客户端 API Key 和模型名称。
...
管理页与凭证写入接口仅允许本机回环地址访问，局域网只开放需要客户端 Key 鉴权的 OpenAI 兼容接口。菜单栏 App 运行期间会自动保证代理可用；代理异常退出时自动恢复。选择“退出”会同时结束后台服务。
